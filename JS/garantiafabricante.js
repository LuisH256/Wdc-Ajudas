const garantias = {
    Fujikura: {
        ...
        mensagem: `
Descaracterização de Garantia:
- Mau uso, marcas de quedas;
- Bateria viciada.

Para solicitar o procedimento e Autorização de envio para o centro de reparo, tenha em mãos:
...
Obs1: Produtos Fujikura, sem comprovação de compra Nacional (Nota fiscal de distribuidor ou revenda autorizada), não serão autorizados para o centro de reparo.
...
Garantia com o fabricante:
suporte.huawei@hdt.energy
Tel: 11 97964-2377
...
WhatsApp (horário comercial): (11) 3035-3777
    ...
    IDEMIA: {
        tempo: '36 meses',
        rma: 'Suporte, com o fabricante, RMA com a WDC com o Fabricante',
        mensagem: `
Suporte do fabricante:
Telefone para atendimento America Latina: +1 714 575 2973
...
Email: sac.brasil@leyardgroup.com
    ...
    Nokia: {
        ...
        rma: 'Suporte e aprovação de Garantia com a WDC',
        mensagem: `
Para suporte e requisição de aprovação para RMA: http://wa.me/551130353777
WhatsApp WDC (horário comercial): (11) 3035-3777
Email: suporte.garantia@wdcnet.com.br
...
function pesquisarGarantia() {
    ...
    const tempoGarantia = document.getElementById('tempo-garantia');
    const rmaFabricante = document.getElementById('rma-fabricante');
    const mensagemErro = document.getElementById('mensagem-erro');
    const mensagemAdicional = document.getElementById('mensagem-adicional');
    const copiarInformacoes = document.getElementById('copiar-informacoes');

    if (garantias[fabricante]) {
        tempoGarantia.innerHTML = `<strong>Tempo de Garantia:</strong> ${garantias[fabricante].tempo}`;
        rmaFabricante.innerHTML = `<strong>RMA:</strong> ${garantias[fabricante].rma}`;
        mensagemErro.style.display = 'none';
        mensagemAdicional.textContent = garantias[fabricante].mensagem.trim() || '';
        copiarInformacoes.style.display = 'block';
    } else {
        tempoGarantia.textContent = '';
        rmaFabricante.textContent = '';
        mensagemErro.style.display = 'block';
        mensagemAdicional.innerHTML = '';
        copiarInformacoes.style.display = 'none';
    }
}

function copiarInformacoes() {
    const mensagemAdicional = document.getElementById('mensagem-adicional').textContent.trim();
    const textarea = document.createElement('textarea');
    textarea.value = mensagemAdicional;
