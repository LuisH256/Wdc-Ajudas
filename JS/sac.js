    ...
    const updateEnvioMaterialEmail = () => {
        ...
        if (destinatarioKey === 'simoes') {
            obsSimoes = `<br><br><span style="color: red;"><b>Observação Importante:</b></span><br>` +
                        `Referente ao processo de devolução para a unidade de <b>Simões Filho/BA</b>, ` +
                        `informamos que é <b>obrigatório o agendamento prévio</b>.<br>` +
                        `Para realizar o agendamento, favor enviar um e-mail para:<br>` +
                        `• iemilli@toplogba.com.br<br>` +
                        `• operacional@toplogba.com.br<br>`;
        }
