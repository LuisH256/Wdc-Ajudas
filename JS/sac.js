...
Cep: 43721-450 SIMOES FILHO/BA`
    ...
    const updateDevolucaoEmail = () => {
        ...
        // Lógica de observação da Matriz adicionada aqui
        let obsMatriz = "";
    ...
    const updatePdAfEmail = () => {
        ...
        let emailText = TEMPLATES.pdaf.replace('{{tipo}}', tipo).replace('{{notas_servico}}', notasServicoMessage).replace('{{nfs}}', nfsMessage).replace('{{ean}}', ean).replace('{{swqt}}', swqtMessage);
    ...
    const updateSolarEmail = (templateKey) => {
        ...
        const emailText = TEMPLATES[templateKey]
            ...
            .replace('{{valorUnitario}}', elements.valor_unitario_input.value || '...')
            .replace('{{quantidade}}', elements.quantidade_input.value || '...')
            .replace('{{ncm}}', elements.ncm_input.value || '...')
            .replace('{{descricao}}', elements.descricao_input.value || '...');
    ...
    const updateTicketParaAdvancedsEmail = (templateKey) => {
        ...
        if (templateKey === 'primeiro_ticket') {
            emailText = emailText.replace('{{produtoDesc}}', produtoDesc).replace('{{nf}}', nf).replace('{{ticket}}', elements.ticket_input.value || '...')
                .replace('{{dataEmissao}}', elements.data_emissao_input.value || '...').replace('{{dataValidade}}', elements.data_validade_input.value || '...');
        } else if (templateKey === 'ticket_expirado') {
            emailText = emailText.replace('{{ticketExpirado}}', elements.ticket_expirado_input.value || '...').replace('{{ticket}}', elements.ticket_input_expired.value || '...')
                .replace('{{dataEmissao}}', elements.data_emissao_input_expired.value || '...').replace('{{dataValidade}}', elements.data_validade_input_expired.value || '...');
        }
        elements.email_content.innerHTML = emailText.trim();
        setVisibility(elements.email_preview, true);
    };
    ...
    const handleTemplateChange = (templateId, value) => {
        ...
        if (templateId === 'sac-template') {
            const sacSubContainersToHide = ['destinatario_container', 'tipo_operacao_container', 'pdaf_options', 'solar_options', 'recusa_nf_options', 'email_preview', 'ticket_correios_options'];
    ...
    if (elements.destinatario) elements.destinatario.addEventListener('change', () => {
        if (elements.sac_template.value === 'envio_material_devolucao') updateEnvioMaterialEmail();
        else if (elements.sac_template.value === 'devolucao') updateDevolucaoEmail();
    });
