...
Cep: 43700-000 SIMOES FILHO/BA`
    ...
    const updateEnvioMaterialEmail = () => {
        const destinatario = DESTINATARIOS[elements.destinatario.value] || '...';
        if (elements.destinatario.value) {
            const emailText = TEMPLATES.envio_material_devolucao.replace('{{endereco}}', destinatario);
    ...
    const updatePdAfEmail = () => {
        ...
        // Lógica de visibilidade e ajuste de texto para AF
        if (tipo === 'AF') {
            emailText = emailText
                .replace(/seguir também com (a nota de serviço|as notas de serviço),/g, '')
                .replace(/EAN .*\n/g, ''); // Remove EAN
            setVisibility(elements.ean_input, false);
            setVisibility(elements.swqt_input, false);
        } else {
            setVisibility(elements.ean_input, true);
            setVisibility(elements.swqt_input, true);
        }
